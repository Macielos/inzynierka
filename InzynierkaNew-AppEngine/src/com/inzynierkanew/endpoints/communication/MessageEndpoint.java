...
import com.inzynierkanew.entities.communication.MessageData;
 ...
 */
@Api(name = "messageEndpoint", namespace = @ApiNamespace(ownerDomain = "inzynierkanew.com", ownerName = "inzynierkanew.com", packagePath = ""))
...
public class MessageEndpoint {
	...
	private static final PlayerEndpoint endpoint = new PlayerEndpoint();
	 ...
	 * 
	 * @param cursorString
	 *            for paging, empty for the first request, subsequent requests
	 *            can use the returned information from an earlier request to
	 *            fill this parameter
	 * @param limit
	...
	public CollectionResponse<MessageData> listMessages(@Nullable @Named("cursor") String cursorString,
	...
	public void sendMessage(@Named("message") String message) throws IOException {
		...
		CollectionResponse<Player> response = endpoint.listPlayer(null, 10);
		for (Player player : response.getItems()) {
			doSendViaGcm(message, sender, player);
		}
	}
	 ...
	 */
	private static Result doSendViaGcm(String message, Sender sender, Player player) throws IOException {
		...
		Result result = sender.send(msg, player.getDeviceRegistrationID(), 5);
		if (result.getMessageId() != null) {
			String canonicalRegId = result.getCanonicalRegistrationId();
			if (canonicalRegId != null) {
				//???
				player.setDeviceRegistrationID(canonicalRegId);
				endpoint.updatePlayer(player);
			}
		} else {
