...
public class WorldGenerationTest {
	...
	public void testLandGenerationNoExistingLands() throws WorldGenerationException {
		/*
		 * NIE DZIA£A 3.0 SEVERE:
		 * com.inzynierkanew.world.WorldGenerationException: Path GraphEdge [13,
		 * 32] - [8, 27] not found! Seed: 2408747665701923831 Map Segment:
		 * 
		 * 
		 * 
		 */
		...
		// -8633507051595676011L - by³y problemy z dopasowaniem wielkoœci landu
		// do istniej¹cego s¹siedniego passage'u
		// -8287553692781528699L - by³y problemy z granicami przy generowaniu
		// landu z istniej¹cym s¹siadem
		// 3.0, 0.1
		// -1017751249576539139L - po lewej droga i crossroads wjecha³y na
		// border pointy

	}
	
	//@org.junit.Test
	public void testLandGenerationMultiGen() throws WorldGenerationException {
		...
		while (true) {
			...
			new WorldDump(mockLandEndpointMultipleGenerationOrders()).dumpTerrain();
			log.info("\n\n\n\n\n");
		}
	}
	
	@Test
	public void testSmoothEdges(){
		WorldGenerator worldGenerator = newWorldGenerator(new int[][]{
			new int[]{
				0, 110, 0, 110, 110, 0, 0
			}, 
			new int[]{
				110, 110, 0, 110, 110, 0, 0
			}, 
			new int[]{
					110, 110, 110, 110, 110, 110, 0
			}, 
			new int[]{
					0, 110, 110, 110, 110, 110, 0
			}, 
			new int[]{
					0, 110, 110, 110, 0, 110, 0
			}, 
			new int[]{
					0, 0, 0, 110, 0, 110, 0
			}, 
		...
		worldGenerator.loadUnitAndMapData();
		log.info(WorldGenerationUtils.mapToString(worldGenerator.getMapSegment()));
		worldGenerator.smoothEdges();
		log.info(WorldGenerationUtils.mapToString(worldGenerator.getMapSegment()));
	}
	...
	public void testLandGenerationSingleNeighbour() throws WorldGenerationException {
		...
		WorldGenerationUtils.init(WorldGenerationUtils.createPrintableSymbols());
		WorldGenerator generator = newWorldGenerator(x); // new
															// Random(-2343597961835384248L));
		generator.generateLand();
	}

	// @org.junit.Test
	public void testLandGenerationMultipleNeighbours() throws WorldGenerationException {
		long x = -5210437816999656336L; // new
										// Random().nextLong();//1234567890L;
	...
	public void testRoadBuilding() throws WorldGenerationException {
		...
		mapSegment[0][4] = 0;
		mapSegment[2][5] = 0;
		WorldGenerator generator = newWorldGenerator(mapSegment); // new
																	// Random(-2343597961835384248L));
	...
	public static FieldTypeEndpoint mockFieldTypeEndpoint() {
		...
		Mockito.when(fieldTypeEndpoint.findByName(Mockito.anyString())).thenAnswer(new Answer<FieldType>() {

			@Override
			public FieldType answer(InvocationOnMock invocation) throws Throwable {
				switch (invocation.getArguments()[0].toString()) {
				case "Grass":
					return new FieldType(100L, "Grass", true, "grass.png");
				case "Mountains":
					return new FieldType(110L, "Mountains", true, "mountains.png");
				case "Road":
					return new FieldType(1L, "Road", true, "road.png");
				case "Passage":
					return new FieldType(2L, "Passage", true, "passage.png");
				case "Town":
					return new FieldType(3L, "Town", true, "town.png");
				case "Dungeon":
					return new FieldType(4L, "Dungeon", true, "dungeon.png");
				}
				return null;
			}
		});
