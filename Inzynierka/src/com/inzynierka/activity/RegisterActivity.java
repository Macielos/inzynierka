...
import com.inzynierka.R;
import com.inzynierka.entities.players.playerendpoint.model.Hero;
import com.inzynierka.entities.players.playerendpoint.model.Player;
import com.inzynierka.messageEndpoint.MessageEndpoint;
import com.inzynierka.messageEndpoint.model.CollectionResponseMessageData;
import com.inzynierka.messageEndpoint.model.MessageData;
		...
		registerListener = new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_DOWN:
					...
					byte[] passwordBytes = passwordTextView.getText().toString().getBytes();
					...
					try {
						...
						Player player = new Player()
								.setName(name)
								.setPassword(RequestValidator.hashPassword(passwordBytes))
								.setHero(new Hero().set("name", name).set("level", 5));
						playerService.registerPlayer(player);
					} catch (Exception e) {
						Log.e(TAG, "Exception received when attempting to register for Google Cloud "
								+ "Messaging. Perhaps you need to set your virtual device's " + " target to Google APIs? "
								+ "See https://developers.google.com/eclipse/docs/cloud_endpoints_android" + " for more information.", e);
						showDialog("There was a problem when attempting to register for "
								+ "Google Cloud Messaging. If you're running in the emulator, "
								+ "is the target of your virtual device set to 'Google APIs?' " + "See the Android log for more details.");
	...
	private class QueryMessagesTask extends AsyncTask<Void, Void, CollectionResponseMessageData> {
		...
		protected void onPostExecute(CollectionResponseMessageData messages) {
			...
			if (exceptionThrown != null) {
				Log.e(TAG, "Exception when listing Messages", exceptionThrown);
